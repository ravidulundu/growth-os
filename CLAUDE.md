...
## Kod Sağlığı ve Okunurluk (Strict)

- **Spagetti kod kesinlikle yasak.** Her fonksiyon tek bir iş yapmalı. 80 satırı geçen fonksiyon bölünmeli.
- **Okunurluk taviz vermez.** Kod, yazan kişi olmadan 6 ay sonra anlaşılabilir olmalı.
- **İç içe derinlik max 4.** 4'ten fazla nesting varsa early return veya extract function uygula.
- **Parametre sayısı max 4.** Fazlası için options/config object pattern kullan.
- **God class/function yasak.** Tek dosyada 300+ satır iş mantığı varsa modüllere böl.
- **Magic number/string yasak.** Sabitler açıklayıcı isimlerle tanımlanmalı.
- **Dead code bırakma.** Yorum satırına alınmış kod, kullanılmayan import/değişken = silmek zorunlu.
- ESLint complexity kuralları (max-depth, complexity, max-params, max-lines-per-function) CI'da zorunlu. Kural ihlali olan PR merge edilemez.
