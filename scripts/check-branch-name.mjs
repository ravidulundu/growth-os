...
const regex = /^(feat|fix|chore|refactor|docs|test|ci|release)\/[a-z0-9._-]+$/;
...
if (!regex.test(branch)) {
  console.error(
    `[branch-check] Invalid branch name '${branch}'. Expected format: <type>/<short-name> where type is feat|fix|chore|refactor|docs|test|ci|release.`
  );
  process.exit(1);
}
